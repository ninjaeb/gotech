...
import { isValidPhoneFormat } from "../src/lib/phone";
...
function contactLabel(contact: {
  ...
  return `${name} <${contact.phone}>${company} (id: ${contact.id})`;
}
...
async function main() {
  ...
  const invalid = contacts.filter((c) => !isValidPhoneFormat(c.phone!));

  const total = await db.contact.count();
  if (invalid.length === 0) {
    console.log(`No contacts with a badly-formatted phone found (${contacts.length} contacts have a phone, out of ${total} total).`);
    return;
  }
  ...
  console.log(
    `Found ${invalid.length} contact(s) with a phone that isn't in the "+countrycode..." format (out of ${contacts.length} with a phone, ${total} total).\n`,
  );
  ...
  if (!values.yes) {
    ...
    console.log("  npm run remove-contacts-with-invalid-phone -- --yes");
    if (withHistory.length > 0) {
