...
function contactLabel(contact: {
  ...
  return `${name}${company} (id: ${contact.id})`;
}
...
async function main() {
  ...
  if (contacts.length === 0) {
    console.log(`No contacts without both a phone and email found (out of ${total} total). Nothing to do.`);
    return;
  }
  ...
  console.log(`Found ${contacts.length} contact(s) with no phone number and no email — no way to reach them.\n`);
  ...
  if (!values.yes) {
    ...
    console.log("  npm run remove-contacts-without-phone-or-email -- --yes");
    if (withHistory.length > 0) {
