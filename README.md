...
1. **A partner fills in their listing** from their portal (*My listing*, `/partner/listing`): company name, tagline, description, services (one per line), industry, website, address, operating hours, and a logo, then submits it for review. The description (About) field supports simple formatting — bold, bullet/numbered lists, links, and images — via a small toolbar and a Preview tab, rather than plain text only. Operating hours are set day by day — an Open/Closed dropdown per day, with time fields when open, Google-My-Business style — rather than typed as free text. Address and operating hours are both optional and shown only when filled in — the detail page adds an embedded map once an address is set, and groups consecutive days sharing the same hours onto one line (e.g. "Monday – Friday: 09:00 – 18:00"). The listing's web address (the `/directory/<slug>` part of its link) is editable from the same page any time; changing it takes effect immediately, so the old link stops resolving.
