...
- **`postinstall` fails with `prisma generate` unable to find `prisma/schema.prisma`** — some cPanel/nodevenv accounts run npm lifecycle scripts with an unreliable working directory (confirmed on this project's own hosting account — see git history around `scripts/postinstall.mjs` and the `$INIT_CWD` fix it replaced). `scripts/postinstall.mjs` resolves the project root from its own file location (`__dirname`) rather than any shell- or npm-provided cwd, specifically so this can't recur; if you still hit it, you're running an older commit — redeploy the latest one.
