...
- **Installable (PWA)** — on a phone, "Add to Home Screen" (iOS Safari) or the browser's own "Install app" prompt (Android/desktop Chrome) puts GoTech on the home screen/app list with its own icon, launching full-screen without browser chrome. Nothing to configure — the manifest, icons, and a minimal service worker (registered for install-eligibility only, no offline caching, so you always see live data) ship with the app
