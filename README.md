...
**Serving `/directory` and `/partner` from your main domain (optional)**: this app has its own host (e.g. `crm.yourcompany.com`), but a public-facing directory reads better on your main marketing domain than a CRM subdomain. To do that, reverse-proxy (not redirect — the address bar should stay on the main domain) `/directory` and `/partner` from the main domain's own web server to this app, forwarding `X-Forwarded-Host`/`X-Forwarded-Proto` (most reverse proxies, including Apache's `mod_proxy`, do this automatically) — this app already builds its own canonical URLs, sitemap entries, and share links from those headers, so they'll correctly say the main domain once it's proxied, with no code change needed for that part. Two things the proxy needs to also cover, or the pages will load unstyled and non-interactive with broken images: this app's own `/_next/*` bundles and its `/api/directory-images/*` route (a listing's photos and logo). And one thing this app itself needs: add the main domain to `experimental.serverActions.allowedOrigins` in `next.config.ts` (already done for `gotka.com`/`*.gotka.com` — adjust if deploying this fork under a different domain) — without it, every form on those pages (partner login, the listing editor, the public lead form) fails Next.js's own CSRF check, since the browser now sends `Origin: https://yourmaindomain.com` instead of this app's own host.
