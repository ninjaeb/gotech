...
- **Quotes** (on a Deal page) — build a quotation from the catalog or a template, with per-line quantity/unit/price, an optional percent or amount discount, a bill-to block prefilled from the deal's contact and company, and terms. Save as a draft, then *Issue* it: that's when it gets its continuous number (`Q-0001`, prefix and padding set in *Settings → Billing*), the issuer details are frozen onto it, and a private share link (`/q/<token>`) is minted. The client opens the link, sees a print-ready document (their browser's print dialog doubles as "Save as PDF"), and accepts — with their name and a tick on the terms — or declines; staff can also record an acceptance that arrived by WhatsApp, email or PO. An accepted quote sets the deal's value to the quote total (toggle in *Settings → Billing*), which the leaderboard and referral commissions read. Issued quotes are immutable: *Revise* clones one as `Q-0001 Rev 2`, which replaces the original when issued (or, if the original was already accepted, when the client accepts the revision); *Withdraw* closes one without deleting its number. Validity dates expire automatically and can be extended; every step lands in the quote's History. Tax (SST) is built in but off until the business is registered — set the rate in *Settings → Billing* and a tax line appears, with a per-line taxable flag. Admin and Sales both quote.
- **Settings → Billing** *(admin)* — business name, registration no., address, contact details and logo printed on documents; tax label/rate/registration no.; quote and invoice number prefixes and digit padding (with a preview of the next number); default quote validity, invoice payment terms, default terms text and payment instructions; and the accepted-quote → deal-value sync toggle.
- **Tasks** — follow-ups and to-dos with due dates and a Low/Medium/High priority (shown as a green/amber/red badge), linked to a contact/company and any number of deals, filterable by Open / Overdue / Due today / Completed and by a minimum linked-deal value, sortable by due date, priority, or deal value (defaults to due date then priority) — your Sort, Assignee, and minimum-deal-value choices are remembered as your own default the next time you open the page, until you change one again. Each task can have any number of Assignees (who's responsible) and any number of Followers (who just want visibility) — both shown right in the task list. Clicking a task opens its own page: full details, its own activity log, and — when the task resolves to a client contact (directly, or through a linked deal/project) — buttons to email or WhatsApp them straight from the task, logged as an activity on both the task and the contact. Each send dialog has a "Draft with AI" button *(optional, requires a Gemini API key)* that writes a first draft grounded in that contact's real history
...
- **Business portal CRM** (`/business-portal` → Companies, Contacts, Deals, Tasks) — a lightweight private CRM for every business-portal (Partner-role) account, entirely separate from the internal CRM above and strictly scoped to the one account that created each record — no partner can ever see another's. Companies and Contacts track basic details and can link to each other; Deals track a value, an Open/Won/Lost status, and an optional company/contact; Tasks track a due date and an optional link to a company, contact, or deal. Unrelated to the referral program or the partner directory above — this is a partner's own private pipeline for the clients *they* manage, not Gotka's. The whole portal, sign-in page included, has its own browser-tab title, description, and share image separate from the internal CRM's, and is marked non-indexable since it all sits behind login
...
1. **A partner fills in their listing** from their portal (*My listing*, `/partner/listing`): company name, tagline, description, products & services, industry, website, address, operating hours, an FAQ, a logo, and optional SEO title/description, then submits it for review. Operating hours sits beside FAQ, to make better use of the page. The description (About) field supports simple formatting — bold, bullet/numbered lists (pressing Enter at the end of a list item continues it, same as most editors), links (pasted), and images (uploaded directly, not just linked) — via a small toolbar and a Preview tab, rather than plain text only, and grows to fit its content instead of scrolling inside a small box; About and Products & Services display side by side here in the editor. Each product/service is its own entry — a title with its price alongside it, and an optional description — added and removed from a small list rather than typed as one line each; shown in full (not just as tags) on the public listing, where a visitor can click any number of them to build up one inquiry covering everything they're interested in. Operating hours are set day by day — an Open/Closed dropdown per day, with time fields when open, Google-My-Business style — rather than typed as free text; the detail page shows them as a day-by-day table (today highlighted). FAQ is the same kind of repeatable list — a question and an answer per entry, shown on the detail page as an expandable Q&A list. A **Rewrite/Generate with AI** button sits next to About, Products & Services, FAQ, and the SEO fields — each improves whatever's already there (never touching a service's price) or drafts fresh from the rest of the listing when the field is empty; About's rewrite in particular only ever expands the draft with more relevant detail, never shortens it, and is written to work for both traditional search engines and AI answer engines. Address is optional and shown only when filled in — the detail page adds an embedded map once it (or Location) is set. The listing's web address (the `/directory/<slug>` part of its link) is editable from the same page any time; changing it takes effect immediately, so the old link stops resolving. An **AI Auto Business Details Creation** section at the top of the editor does the whole first draft in one go, and owns the Website field itself — it's both what a Google Maps pick fills in and what AI Auto Create reads when there's no pick: search for the business on Google Maps (needs `GOOGLE_PLACES_API_KEY` — a Google Maps Platform key with *Places API (New)* enabled; see `.env.example` for exactly how to restrict it) searches live as a partner types, no button to press; picking a result fills in Website. **AI Auto Create** then reads the Google listing (its category, summary, reviews) and the website itself (the homepage plus its about/services/FAQ pages) and fills in About, tagline, Products & services, FAQ, industry, business categories, operating hours, and address — hours and address copied straight from Google, the rest AI-written — all as a draft to review before saving. Without the Google key, the section still shows a plain Website field and AI Auto Create works from that alone; without `OPENROUTER_API_KEY` the whole section doesn't show, and Website is a plain field in the form instead, same as before this existed.
2. **An admin approves it** from *Settings → Directory*. Approving snapshots the current draft onto the public page — the partner can keep editing afterwards without taking the live listing down; only submitting for review again (or an admin unpublishing it) changes what's public. The same page's listings table can also **transfer** a listing to a different partner account (e.g. a duplicate signup, or the business changed hands) — pick the new owner and confirm; everything else about the listing (status, public page, leads) stays exactly as it was.
...
**SEO and search**: a listing's detail page carries Open Graph/Twitter metadata, a canonical URL, and schema.org `LocalBusiness` structured data (name, description, address, services with their own description, and a link back — never a phone number, which stays internal as above) for both search engines and AI answer engines. A service's price (shown normally on the page itself) is deliberately left out of this structured data: partners type it as free text ("RM 25/day", "From RM 900/mo"), not the plain decimal schema.org's `Offer.price` expects, and a value it can't parse as one just makes that part of the markup invalid. When an FAQ is set, the page also carries a separate schema.org `FAQPage` structured data block (each entry as its own `Question`/`acceptedAnswer` pair) — a standard source for search-result rich snippets, and grounded Q&A content for AI answer engines to quote directly. The page also carries a visible breadcrumb trail (directory home › category › business name) matching its own schema.org `BreadcrumbList` structured data — a real, crawlable link back to the category and directory home, not only a JSON-LD block a browser never renders — and, when the listing has a category, a *More businesses in [category]* section below its own content linking to a handful of others in it (see `relatedListingsByCategory` in `src/lib/directory.ts`), so landing on one listing page directly has a path to nearby ones without going all the way back through search. The title/description a partner sets under *Search & social preview* (optionally AI-drafted) take priority; otherwise it falls back to the tagline/About text, same as before that field existed. The social preview image — and the structured data's own image — is the partner's actual logo (served from their approved listing, never a live unapproved edit) rather than Gotka's generic app icon, now that logos are re-served under a real URL instead of the inline `data:` URL they're stored as (which Open Graph/Twitter/JSON-LD can't reference directly). The home page and every listing also carry schema.org `CollectionPage`/`ItemList` structured data listing what's on the page. Set `GOOGLE_SITE_VERIFICATION` / `BING_SITE_VERIFICATION` (the tokens from each console's "HTML tag" method) to verify the site in Google Search Console and Bing Webmaster Tools without committing anything host-specific, and `INDEXNOW_KEY` to have every publish, unpublish and slug change pinged to Bing (and through it Copilot and ChatGPT search), Yandex, Naver and Seznam via [IndexNow](https://www.indexnow.org/) the moment it happens — the key is served at `/indexnow-key.txt` to prove ownership. `PLAUSIBLE_DOMAIN` adds Plausible Analytics to the directory pages only (never the CRM or the portals); in Plausible, filter *Referrer* by `chatgpt.com`, `perplexity.ai`, `copilot.microsoft.com`, `claude.ai` or `gemini.google.com` to see visits that came from an AI answer.

Every business category has its own friendly, crawlable page too — `/<locale>/business/category/<slugified-name>` (e.g. `/en/business/category/printing-publishing`) — rather than only being reachable through the search box's category dropdown. Only a category with at least one published business is treated as a real page: those are linked (with a listing count) from a *Browse by category* section below the home page's grid and listed in `/sitemap.xml` and `/llms.txt`; a category nobody has published into yet still renders, but is marked `noindex` and left out of both files until a listing carries it, so search engines aren't fed dozens of near-empty pages. (An earlier version rendered every category as a pill link right under the search bar; removed for being too much clutter above the fold before any actual listing shows — the section now sits below the listings instead.) Each category page also carries the same visible breadcrumb trail back to the directory home, and, below its own results, a *Browse other categories* list linking to every other populated category — before this, a sibling category was only reachable by going back to the home page's own grid.

Every state or region at least one published listing carries gets the same treatment — `/<locale>/business/location/<slugified-state>` (e.g. `/en/business/location/selangor`), linked (with a count) from a *Browse by location* section on the home page, listed in `/sitemap.xml` and `/llms.txt`, and carrying its own breadcrumb trail and a *Browse other locations* list of every other state. Unlike category, state isn't a separate admin-managed table — a listing's `state` field is free text — so there's no such thing as an "empty" location page to keep `noindex`: a state's own page exists exactly when, and only when, a published listing carries it (see `findStateBySlug`/`countListingsByState` in `src/lib/directory.ts`). A listing's own "Visit us" state now links here too, instead of the home page's grid pre-filtered by a `?state=` query parameter, which had no title, description, structured data, or sitemap entry of its own — country stays a query-param filter only (every listing so far is in Malaysia, so a country-level page would just duplicate the home page).

The home page itself carries schema.org `WebSite` (with a `SearchAction` for its search box) and `Organization` (Gotka Technologies as publisher) structured data, an enriched `CollectionPage`/`ItemList` where each entry is a `LocalBusiness` with its logo, region and one-line summary, and a `FAQPage` for its own FAQ; below the grid it renders a short description of the directory, the category links above, a three-step *How it works*, a *List your business* call-out and that FAQ, in all three languages — indexable text and crawlable links for search engines, quotable self-contained statements for AI answer engines (see `src/lib/directory-home-copy.ts`). Every directory page declares `x-default` alongside its language alternates, sets `googlebot` directives allowing full-size image previews, and names *Gotka Business Directory* as its site; the home, category and sign-up pages share a generated 1200×630 Open Graph/Twitter image (`src/app/[locale]/business/opengraph-image.tsx`), which a listing without a logo inherits too. Partner logos are served by URL (`/api/directory-images/logo/<slug>?v=<publish time>`, cached for a year since the URL changes on every re-approval) rather than inlined as `data:` URLs — which had made the home page ~870KB for five listings — and the grid only sends the browser the fields it shows and searches. The Chinese and Malay pages set `<html lang>` accordingly: the locale segment is forwarded to the root layout as a request header by `src/proxy.ts`, since nothing below the root layout can set that attribute. `/favicon.ico` is served (a rewrite to the app icon) rather than 404ing, and a bare `/en`, `/zh` or `/ms` redirects to that language's directory home.

The whole directory — home page, every listing, every category page — carries its language as a leading URL segment: `/en/directory`, `/zh/directory/some-company`, `/ms/directory/category/web-design`, and so on. All three languages are equally real, separately indexable pages this way, not query-param or cookie-only variants of one canonical URL. The language switcher navigates to the same page under the new locale segment (and still sets a `directory_locale` cookie alongside it, read by the bare, un-prefixed `/directory/*` paths below and by `/business/login`, which shares the same header but isn't part of the locale-prefixed tree itself). Every directory page declares its other language versions as `hreflang` alternates of itself, and `/sitemap.xml` lists all three language URLs (plus an `x-default` alternate) for the home page, every populated category page, every location page, and every listing — a listing entry also names its logo via the [Google Images sitemap extension](https://developers.google.com/search/docs/crawling-indexing/sitemaps/image-sitemaps), and a category or location entry's `lastmod` is the most recent publish date among its listings (only listings had one before). The old bare `/directory`, `/directory/<slug>`, `/directory/signup`, and `/directory/category/<name>[/zh|/ms]` URLs (from before this scheme existed) still work — each is now a permanent redirect into its locale-prefixed equivalent, so old bookmarks and search results don't break. `/robots.txt` allows `/directory` and each locale-prefixed variant, and disallows the rest of the app (everywhere else already requires a login, so this is belt-and-suspenders). The directory's own search box filters as you type (matching a service's title or description too), with no page reload.
...
Everything the deploy does is appended to `deploy.log` in the Application root (already covered by `.gitignore`) — check there first if a push doesn't seem to have taken effect. **The deploy also tests itself**: after signalling the restart, the script keeps polling `/api/health` (a public JSON probe — which commit the running build was made from, whether the database answers, whether `/public/sitemap.xml` and `/public/llms.txt` are real or still their committed placeholders) until the app reports the new commit as built, then checks what a broken deploy has actually broken before: the served `/sitemap.xml`, `/zh/business` rendering with `lang="zh"`, the directory's share image, one listing logo (the `og:image` route handler), a referral redirect, and that Googlebot/bingbot get a 200 (the AI crawlers — GPTBot, OAI-SearchBot, ClaudeBot, PerplexityBot — are only warned about, since a block there is usually a host rule). It needs the site's public origin: `DEPLOY_SMOKE_URL` (or `SITE_URL`); with neither set it's skipped. A failure can't undo the deploy — the restart already happened — but it lands in `deploy.log` with the reason instead of leaving an old build serving silently. Since this runs `npx prisma migrate deploy` unattended on every push that changes the schema, a migration goes live the moment its commit reaches `DEPLOY_BRANCH` — the same way this repo's own workflow already pushes straight to a shared branch with no review gate, just now automatic instead of a manual step you'd otherwise run right after.
...
- **Build fails on `/icon` or `/apple-icon` with `vips2png: unable to write to target` / `glib: Error creating thread: Resource temporarily unavailable`** — the favicon is rendered at build time via `next/og`, which briefly needs to spawn a native image-processing thread; on tightly resource-capped shared hosting that can momentarily fail if something else on the account is busy at that exact instant. `server.js` keeps the previous build serving when this happens and retries the build by itself — it signals a restart 5 minutes later, up to 3 attempts per commit (the count is in `.next-build-retry.json`, cleared by the next successful build), logged in `stderr.log` — so it's expected to be transient and self-heal; only worth investigating further if it fails on every single deploy rather than occasionally.
- **`/sitemap.xml` or `/llms.txt` serve a `Placeholder` comment with no URLs** — both are static files in `/public`, committed as placeholders and overwritten with real content when the app starts (`src/instrumentation.ts`) and on every publish/unpublish. Seeing the placeholder means the app process that should have written them never started on this commit: usually `next build` failed after a deploy and `server.js` restored the previous build (the web server still serves the freshly checked-out placeholder files straight from disk, so the two get out of step). `GET /api/health` says which commit is actually built versus checked out; `stderr.log` has the build error. The deploy's own smoke test (above) fails loudly in `deploy.log` in exactly this situation, and `server.js` retries the build on its own a few times.
- **Route handlers answer `500` after ~10 seconds while pages work** (`/api/directory-images/logo/...`, `/r/<code>`, `/api/whatsapp/webhook`) — a starved connection pool. Next.js compiles a server module once per bundling layer it's imported from, so the pages and the route handlers each got their own `PrismaClient` and pool, and with the driver keeping every pool's full `connectionLimit` open even when idle, the second pool could get no connections at all on an account with a small `max_user_connections`; every query through it then waited out the driver's 10 s `acquireTimeout` and failed. `src/lib/db.ts` now shares one client per process (cached on `globalThis` in production too), keeps only one idle connection per pool, and fails a starved query after 5 s so the symptom can't hide behind a slow page. If it still happens, the account is out of connections for some other reason — see the `pool timeout` entry below.
- **An AI crawler (`GPTBot`, `PerplexityBot`, ...) gets an empty `429` or `403` while browsers, Googlebot and bingbot get `200`** — the block is at the host, not in this app: `robots.txt` allows everything except `/system`, and Cloudflare passes these crawlers through. On cPanel it's usually Imunify360's anti-bot rules or a LiteSpeed/`.htaccess` rule matching the user agent. The deploy smoke test (above) logs each blocked crawler as a warning. If the directory is meant to be found and cited by AI answer engines, whitelist the crawler's user agent there (or ask the host to) — blocking `GPTBot` only keeps the pages out of OpenAI's *training* data; `OAI-SearchBot`/`ChatGPT-User` are what ChatGPT search reads.
