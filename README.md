...
Every business category has its own friendly, crawlable page too — `/directory/category/<slugified-name>` (e.g. `/directory/category/printing-publishing`) — rather than only being reachable through the search box's category dropdown; the home page links to each one directly (not just via its `<select>`, which a crawler can't follow) so they're discoverable on-page, not only through the sitemap. A category page's own language lives right in its URL — `/directory/category/<name>/zh` or `/directory/category/<name>/ms`, English being the bare URL — rather than a query parameter, since each is otherwise a clean, single-purpose landing page for one category. The home page and every listing instead take `?lang=en|zh|ms` (there's no single, clean path segment to put it in on a page that isn't about one specific thing) to render a specific language directly — the language switcher itself still just sets a cookie, but a crawler carries no cookie, so this is what makes a specific URL actually render in a specific language for indexing. Every directory page declares its other language versions as `hreflang` alternates of itself. `/directory`, every category page, and every listing (each with its language variants) are the only pages included in `/sitemap.xml`; `/robots.txt` allows just `/directory` and disallows the rest of the app (everywhere else already requires a login, so this is belt-and-suspenders). The directory's own search box filters as you type (matching a service's title or description too), with no page reload.
