...
- **A red "Error" popup after "Run NPM Install"** — cPanel's UI surfaces anything npm writes to stderr in a red box, regardless of whether the install actually failed. `postinstall` runs `scripts/postinstall.mjs`, which folds Prisma's own routine stderr notices (`Loaded Prisma config...`, `Prisma schema loaded...`) into stdout so they stop triggering this; if you still see the popup, check for an `npm error` line specifically (an actual failure — e.g. `npm error command failed` — always lands on stderr regardless of that redirect, so it'll still show up here) — an `npm warn deprecated ...` line with no `npm error` line means the install succeeded and this is once again just cosmetic noise from some other dependency.
- **`postinstall` fails** (`prisma generate` can't find `prisma/schema.prisma`, or `npm error ... Cannot find module '.../scripts/postinstall.mjs'`) — some cPanel/nodevenv accounts, including this project's own hosting account, run npm lifecycle scripts from an unreliable working directory — confirmed to be the nodevenv's own internal lib folder (`/home/USER/nodevenv/DOMAIN/24/lib`) rather than the project directory, specifically via cPanel's "Run NPM Install" button. `scripts/postinstall.mjs` resolves the project root from its own file location (`__dirname`) rather than any shell- or npm-provided cwd, and `package.json`'s `postinstall` command finds that file itself via `$npm_config_local_prefix` (an absolute path) rather than a plain relative one — see the comment at the top of `scripts/postinstall.mjs` for the full history and why. If you still hit either failure, you're running an older commit — redeploy the latest one.
...
scripts/
  ...
  postinstall.mjs           Regenerates the Prisma Client after npm install (see package.json's postinstall)
