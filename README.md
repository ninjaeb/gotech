...
1. **A partner fills in their listing** from their portal (*My listing*, `/partner/listing`): company name, tagline, description, services (one per line), industry, website, address, operating hours, and a logo, then submits it for review. Address and operating hours are both optional and shown only when filled in — the detail page adds an embedded map once an address is set. The listing's web address (the `/directory/<slug>` part of its link) is editable from the same page any time; changing it takes effect immediately, so the old link stops resolving.
