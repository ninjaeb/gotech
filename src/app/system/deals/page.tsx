  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Link href={`/system/deals/new?pipelineId=${selectedPipeline.id}`} className={buttonClasses()}>
      ...
      {pipelines.length > 1 && (
        <div className="mb-4 flex flex-wrap gap-1.5">
          {pipelines.map((pipeline) => (
            <Link
              key={pipeline.id}
              href={`/system/deals?pipeline=${pipeline.id}`}
              className={cn(
      ...
      {flagged && (
        <div className="mb-4 flex items-center gap-2 text-sm text-orange-700 dark:text-orange-400">
          ...
          <Link
            href={`/system/deals?pipeline=${selectedPipeline.id}`}
            ...
            <div className="min-h-16 space-y-2 rounded-lg bg-slate-100/70 p-2 dark:bg-neutral-900/60">
              {column.deals.length === 0 ? (
                ...
                column.deals.map((deal) => {
                  ...
                  >
                    <Link href={`/system/deals/${deal.id}`} className="block">
