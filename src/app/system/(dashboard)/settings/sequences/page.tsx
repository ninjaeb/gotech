...
export default async function SequencesPage() {
  ...
  return (
    <div>
      <PageHeader
        breadcrumbs={[{ label: "Settings", href: "/system/settings" }, { label: "Sequences" }]}
        title="Sequences"
        description="Multi-step automated email cadences you can enroll a contact into"
        actions={
          <Link href="/system/settings/sequences/new" className={buttonClasses("primary", "sm")}>
              ...
              {sequences.map((sequence) => (
                <li key={sequence.id} className="flex items-center justify-between gap-2 py-2.5 text-sm">
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    <Link
                      href={`/system/settings/sequences/${sequence.id}`}
                      title="Edit sequence"
