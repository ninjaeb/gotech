...
export default async function TaskDetailPage({
  ...
  const clientSection = (() => {
    ...
    return (
      <span className="inline-flex items-center gap-1.5">
        <Link href={`/system/contacts/${clientContact.id}`} className="text-indigo-600 hover:underline">
          {name}
        </Link>
  ...
  return (
    <div>
      <PageHeader
        breadcrumbs={[{ label: "Tasks", href: "/system/tasks" }, { label: task.title }]}
        title={task.title}
        description={
        ...
        actions={
          canManage && (
            <>
              ...
              <Link href={`/system/tasks/${task.id}/edit`} className={buttonClasses("secondary")}>
            ...
            <CardBody className="space-y-4">
              ...
              <div className="grid gap-3 text-sm sm:grid-cols-2">
                ...
                <DetailRow
                  ...
                  href={task.project ? `/system/projects/${task.project.id}` : undefined}
                />
                <DetailRow
                  label="Assignees"
