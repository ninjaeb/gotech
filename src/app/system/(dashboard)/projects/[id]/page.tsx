...
export default async function ProjectDetailPage({
  ...
  return (
    <div>
      <PageHeader
        breadcrumbs={[{ label: "Projects", href: "/system/projects" }, { label: project.name }]}
        title={project.name}
        description={
          <Link href={`/system/deals/${project.deal.id}`} className="text-indigo-600 hover:underline">
          ...
          <Card>
            ...
            <CardBody className="grid gap-4 text-sm sm:grid-cols-2">
              ...
              <DetailRow
                ...
                href={project.deal.company ? `/system/companies/${project.deal.company.id}` : undefined}
              />
              <DetailRow
                label="Contact"
                value={
                ...
                href={project.deal.contact ? `/system/contacts/${project.deal.contact.id}` : undefined}
              />
            </CardBody>
          </Card>
          ...
          {canManage && (
            <Card>
              <CardHeader>
                ...
                <Link href={`/system/projects/${project.id}/invoices/new`} className={buttonClasses("secondary", "sm")}>
                    ...
                    {project.invoices.map((invoice) => (
                      <li key={invoice.id} className="flex flex-wrap items-center justify-between gap-3 py-2.5 text-sm">
                        <div className="min-w-0">
                          <Link
                            href={`/system/projects/${project.id}/invoices/${invoice.id}/edit`}
