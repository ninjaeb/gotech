...
// Same prompt/schema/flow as scanBusinessCard (src/app/actions/scan-business-card.ts),
// duplicated rather than shared because the two write to different tables
// under different auth — a partner's card scan must only ever match/create
// their own PartnerCompany rows, never the system-wide Company table.
const SYSTEM_PROMPT =
...
export async function scanPartnerBusinessCard(formData: FormData): Promise<ScanCardResult> {
  ...
  const website = parsed.companyWebsite.trim() || (parsed.email ? deriveCompanyDomain(parsed.email) : null);
  const company = await findOrCreatePartnerCompanyByName(partner.id, parsed.companyName, {
    website,
