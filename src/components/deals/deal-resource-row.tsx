...
import { deleteDealResource, updateDealResource } from "@/app/actions/deal-resources";
import { Input } from "@/components/ui/field";
import { Button } from "@/components/ui/button";
import { ConfirmSubmitButton } from "@/components/ui/confirm-submit-button";

export function DealResourceRow({
  dealId,
  resource,
}: {
  dealId: string;
        ...
        <form
          action={(formData) => {
            startTransition(async () => {
              await updateDealResource(dealId, resource.id, formData);
              setEditing(false);
            });
          }}
  ...
  return (
    <li className="flex items-center justify-between gap-3 py-2.5">
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <form action={deleteDealResource.bind(null, dealId, resource.id)}>
          <ConfirmSubmitButton
