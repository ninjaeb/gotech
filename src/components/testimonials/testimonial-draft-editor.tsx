...
import { regenerateTestimonialDraft, rewriteAdminDraft, updateTestimonialDraft } from "@/app/actions/testimonials";
...
export function TestimonialDraftEditor({
  ...
  const busy = saving || regenerating || rewriting;
  ...
  // Discards whatever's in the box and starts fresh from the contact's
  // context — persists immediately, unlike Rewrite below.
  ...
  // Polishes whatever's currently in the box (including a staff hand-edit)
  // instead of discarding it. Only updates the textarea, not the DB — same
  // as a manual edit, it's still just a draft until "Save draft" is clicked.
